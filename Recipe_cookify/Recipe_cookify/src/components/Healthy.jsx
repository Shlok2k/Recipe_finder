...
const Healthy = () => {
    ...
    const getHealthy = async () => {
        try {
            ...
            if (check) {
                try {
                    ...
                    if (Array.isArray(parsedData) && parsedData.length > 0 && parsedData[0].idMeal) {
                        setHealthy(parsedData);
                        setLoading(false);
                        return;
                    }
                } catch (parseError) {
                    console.error('Error parsing stored data:', parseError);
                    localStorage.removeItem('healthy');
                }
            }
            ...
            if (!api.ok) {
                throw new Error(`Failed to fetch healthy recipes: ${api.status}`);
            }

            const data = await api.json();

            if (!data.meals || data.meals.length === 0) {
                throw new Error('No recipes found for healthy');
            }
            ...
            localStorage.setItem('healthy', JSON.stringify(validDetailedMeals));
            setHealthy(validDetailedMeals);
    ...
    if (loading) {
        return (
            <div className="loading-container">
                <div className="loading">Loading healthy recipes...</div>
            </div>
        );
    }
        ...
        return (
            <div className="error-container">
                <div className="error-message">
                    ...
                    <p>{error}</p>
                    <button onClick={getHealthy} className="retry-button">
                        Try Again
                    </button>
                </div>
            </div>
                                ...
                                <div className="image-container">
                                    <img 
                                        ...
                                        onError={(e) => {
                                            ...
                                            e.target.src = 'https://via.placeholder.com/300x200?text=Healthy+Recipe';
                                        }}
                                    />
                                </div>
                                <h4>{meal.strMeal}</h4>
